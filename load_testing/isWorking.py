...
def create_client(request, room_name, room_pass, port, is_join_client):
    try:
        audio_port, ack_port, rr_port = send_data_to_server(request, room_name, room_pass, 8000)
...
def monitor_system(interval):
    while not terminate_event.is_set():
        ...
        core_0_cpu_percent = cpu_percents[0]
        memory_percent = psutil.virtual_memory().percent
        
        # Write to file or print to console
        # print(f"Timestamp: {timestamp}, Core 0 CPU Percent: {core_0_cpu_percent}%, Memory Percent: {memory_percent}%")
        
        global max_cpu_percent
        global max_memory_percent
        if core_0_cpu_percent > max_cpu_percent:
            max_cpu_percent = core_0_cpu_percent
...
def signal_handler(sig, frame):
    ...
    for room_name, clients in clients_in_rooms.items():
        max_rooms = max(max_rooms,len(clients))
    ...
    print("")
    print("Peak CPU Utilization :",max_cpu_percent)
    print("")
    print("Peak Memory Utilization: ",max_memory_percent)
    print("")
    print("Average Join Time:", sum(total_join_time) / len(total_join_time) if total_join_time else 0)
    print("")
    std_dev_join_time = statistics.stdev(total_join_time) if len(total_join_time) > 1 else 0
    print("Std dev of join time :",std_dev_join_time)
    ...
    print("")
    print("Max clients in a room: ",max_rooms)
    ...
    print("")
    print("Total_sent_packets = ", total_sent_packets)
    ...
    print("Ideal Throughput for ",num_join_clients,"clients : ",throughput_mbps)
...
if __name__ == "__main__":
    ...
    num_join_clients = 200  # Number of clients to send "join" requests
