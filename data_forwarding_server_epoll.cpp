...
map<string, vector<pair<string, int>>> rr_IP_ports;                // new temp mapping for rr ip and ports
...
//     string response_str = "hello from server\n";
...
int handleTCPSocketEvent(int socket_fd, int epoll_fd)
{
    ...
    //------------------------------------------------------------------------------------------------------------------------------
    cout << "printing the extracted info"
         << " " << ReqType << " " << Room_id << " " << Pwd << " " << ip_address << endl;
    json response;
    if (ReqType == "Create")
    {
        ...
        audioSocket_RoomID[audio_socket] = Room_id; // The audio data needs to be broadcasted to all the clients in the room. Therefore we need to keep a mapping from audioSocket of each room to it's roomID.
        ackSocket_RoomID[ack_socket] = Room_id;     // Same idea, whenever an ack packet comes to the server, it should be able to find which room does the ack packet belongs to.
...
void handleUDPSocketEvent(int room_fd) // room sockets include audio, ack and rr sockets
{
                                     ...
                                     // also, it won't be the entire audio buffer, only a part that contains the meta_data about the packet.
    {
        ...
        is_host = 0;
        // cout<<"Audio dummy received for room ID : "<<roomid<<", list of IP ports in this room: "<<endl;
        // for (const auto &clientIP : roomID_IP_ports[roomid])
        // {
        //     cout << "client_ip: " << clientIP.first << " client_port: " << clientIP.second.first << endl;
        // }
    ...
    else if (!strcmp(test_ack, buffer))
    {
        map<int, string>::iterator it;
        string roomid;
        it = ackSocket_RoomID.find(room_fd);
        if (it != ackSocket_RoomID.end())
        {
            roomid = ackSocket_RoomID[room_fd];
        }
        // map<int,pair<pair<string,int>,pair<pair<string,int>,pair<string,int> > > > reference_table; // (ssrc-----------> ((audio_ip_address,audio_port number),((ack_ip,ack_port),(rr_ip,rr_port))))
        ack_IP_ports[roomid].push_back(make_pair(sender_ip, sender_port));
        // cout<<"Ack dummy received for room ID : "<<roomid<<", list of IP ports in this room: "<<endl;
        // for (const auto &clientIP : ack_IP_ports[roomid])
        // {
        //     cout << "client_ip: " << clientIP.first << " client_port: " << clientIP.second << endl;
        // }
    ...
    else if (!strcmp(test_rr, buffer))
    {
        string roomid;
        map<int, string>::iterator it;
        it = rrSocket_RoomID.find(room_fd);
        if (it != rrSocket_RoomID.end())
        {
            roomid = rrSocket_RoomID[room_fd];
        }
        rr_IP_ports[roomid].push_back(make_pair(sender_ip, sender_port));
        // cout<<"rr dummy received for room ID : "<<roomid<<", list of IP ports in this room: "<<endl;
        // for (const auto &clientIP : rr_IP_ports[roomid])
        // {
        //     cout << "client_ip: " << clientIP.first << " client_port: " << clientIP.second << endl;
        // }
